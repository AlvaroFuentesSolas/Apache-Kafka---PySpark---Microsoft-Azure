...
# Group the data by window and SHOP_CITY and compute the count of sales in each city
salesCounts = df \
    .withWatermark("CLOSED_DATE", "1 hour") \
    .groupBy(window(df.CLOSED_DATE, "1 hour", "1 hour"), df.SHOP_CITY) \
    .count()
...
def foreach_batch_function(df, epoch_id):
    ...
    for row in data:
        ...
        cursor.execute("DELETE "
                       ...
                       "WHERE SHOP_CITY ='" + str(row['SHOP_CITY']) +
                       "' AND TIMESTART='" + str(windowTime['start']) +
                       "' AND TIMEEND='" + str(windowTime['end']) + "'")

        cursor.execute("INSERT INTO dbo.SALESBYCITY("
                       "[TIMESTART],"
                       "[TIMEEND],"
                       "[SHOP_CITY],"
                       "[CUENTA])"
                       "values(?, ?, ?, ?)",
                       windowTime['start'], windowTime['end'], row['SHOP_CITY'], row['count'])
...
salesCounts.writeStream.foreachBatch(foreach_batch_function).outputMode("update").start().awaitTermination()
